...
* 版本�?		日期	  作�?			说明
*  2.7  	2025-09-27	黄仕�?  	 创建该文�?
****************************************************************************/
#include "ocd_mpu6050.h"
/**
 * @brief  读取MPU6050指定寄存器的数据
 * @param  _tMPU6050 MPU6050的句柄指�?
 * @param  Reg       要�?�取的寄存器地址
 * @retval 寄存器�?�取到的数据
 */
...
uint8_t OCD_MPU6050_Init(tagMPU6050_T *_tMPU6050)
{
    ...
    // 设置陀螺仪量程 ±2000dps（GYRO_CONFIG寄存�?�?0x1B�?0x18�?
    S_MPU6050_Write(_tMPU6050, MPU6050_GYRO_CONFIG, 0x18);
    Drv_Delay_Ms(20);

    // 设置加速度计量�? ±2g（ACCEL_CONFIG寄存�?�?0x1C�?0x00�?
    S_MPU6050_Write(_tMPU6050, MPU6050_ACCEL_CONFIG, 0x00);
    Drv_Delay_Ms(20);

   // 设置采样率分频（SMPLRT_DIV寄存�?�?0x19�?0x07，采样率1kHz/(1+9)=100Hz�?
    S_MPU6050_Write(_tMPU6050, MPU6050_SMPLRT_DIV, 0x09);
    Drv_Delay_Ms(20);

   // 设置低通滤�?（CONFIG寄存�?�?0x1A�?0x03�?
    S_MPU6050_Write(_tMPU6050, MPU6050_CONFIG, 0x03);
    Drv_Delay_Ms(20);

    // 配置 INT 引脚为低电平有效，推挽输出，保持高电平（INT_PIN_CFG寄存�?�?0x37�?0x80�?
    ...
    // 使能数据就绪�?�?（DATA_RDY_EN=1�?
    S_MPU6050_Write(_tMPU6050, MPU6050_INT_ENABLE, 0x01);
    Drv_Delay_Ms(1);
    
    // 检�?MPU6050�?否�?�常（�?�取WHO_AM_I寄存�?�?0x75，默认�?0x68�?
...
{
	return S_MPU6050_Read(_tMPU6050,MPU6050_WHO_AM_I);		//返回WHO_AM_I寄存器的�?
}

/**
 * @brief  获取MPU6050的加速度和�?�速度数据
 * @param  _tMPU6050 MPU6050的句柄指�?
 * @retval �?
 */
...
//     if (abs_pitch < 5.0f) {
//         alpha = 0.90f;  // 小�?�度：加速度计权�?0.10
//     } else if (abs_pitch < 15.0f) {
//         alpha = 0.95f;  // �?角度：加速度计权�?0.05
//     } else if (abs_pitch < 30.0f) {
//         alpha = 0.98f;  // 大�?�度：加速度计权�?0.02
//     } else {
...
//     _tMPU6050->stcAngle.ConYaw = _tMPU6050->stcAngle.ConYaw + _tMPU6050->stcGyro.ConGyroZ * dt; // 无加速度计数�?�?�?
// }
float acc_roll;
float acc_pitch;
